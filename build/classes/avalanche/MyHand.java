...
public int call=-1, bet = -1, chips = 0,	lastCall = -1, playerDefaultPos = 4, initOppons = 0;
...
//	System.out.println("updateVisuals");
	...
	if (canvas!=null) {
		...
		try {
			if (chipsRegion.rawString.length()>0) chips = Integer.parseInt(chipsRegion.rawString);
		...
		if ((betRegion.rawString.contains("call") || raiseinput.rawString.contains("call"))) {
			strCall = "Call EV:";
				...
				if (call>0) {
					if (!playerActionSubmitted) EngineLogical.dispatcher.dispatchEvent(new AvalEvent(AvalEvent.REQUEST_SIMULATION, this));

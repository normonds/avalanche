...
public static void initListeners () {
	...
	dispatcher.addEventListener(AvalEvent.CHANGE_MY_CHIPS, changeMyChips);
	...
	dispatcher.addEventListener(AvalEvent.FORCED_SIMULATION, forcedSimulation);
}
...
public static EveListener changeMyChips = new EveListener () {		@Override public void onEvent (Eve e) { //Event clientEve=(Event) e;
	//trace("listener:pauseLoop");
	//trace("request simulation");
	//calculateEquity();
	trace("CHIP CHANGE forHand:" + me.chipHandDifference + " (last:" + me.chipLastDifference + ")");
}};
...
public static EveListener myHandChange = new EveListener () { 	@Override public void onEvent (Eve e) {//Event clientEve=(Event) e;
	...
	me.chipHandDifference = 0;
	me.chipLastDifference = 0;
	me.chipsInPlay = 0;
...
public static EveListener forcedSimulation = new EveListener () { 			@Override public void onEvent (Eve e) {//Event clientEve=(Event) e;
	//trace("  listener:pot change "+board.validPot);
	//History.two += "kettle:"+board.validPot+";";
	//EngineLogical.dispatcher.dispatchEvent(new AvalEvent(AvalEvent.FORCED_SIMULATION, this));
	String hand = me.validHand;
	int oppons = Opponent.activeOppons()>0 ? Opponent.activeOppons() : Opponent.lastActiveOppons;

	if (me.validHand.equals("")) hand = me.lastValidHand;
	if (me.lastValidHand.equals("")) {
		Avalanche.trace(me.validHand);
		Avalanche.trace(me.lastValidHand);
		return;
	}

	calculateEquityRaw (hand, oppons);
}};
...
public static void calculateEquityRaw (String hand, int oppons) {
...
public static void calculateEquity (String str) {
	trace("calculateEquity:" + boardHandOppon + " -- " + lastBoardHandOppon);
	...
	if (!boardHandOppon.equals(lastBoardHandOppon)) {
		if (me.hasHand()) {
			...
			if (Opponent.activeOppons()<1) {
				...
				calculateEquityRaw (me.validHand, Opponent.activeOppons());
				//Avalanche.clearEquityLabel(me.validHand + " " + board.validCards + " ("+Opponent.activeOppons()+")");
				//Avalanche.reqMonteCarlo(me.validHand+""+Opponent.opponStr(), board.validCards, "");
				//boardHandOppon
